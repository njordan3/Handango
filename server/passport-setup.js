...
module.exports = function(passport) {
    ...
    passport.deserializeUser(function(usr, done) {
        db.findUser(usr.username, usr.id, false)
            .then(function(row) {
                if (row.sqlMessage) { return Promise.reject(`User ${usr.username} not found in Deserialize`); }
                else { done(null, {username: row.email, id: row.external_id}); }
            })
            ...
            db.connectToDB()
                ...
                .then(function(row) {
                    if (req.session.passport.user.secret !== null) {
                        return Promise.resolve({username: row.email, type: row.external_type, id: row.external_id, secret: row.twofactor_secret, ejs: {successRedirect: '/logout'}});
